...
TEST(BraveWalletUtilsUnitTest, Mnemonic) {
  ...
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    EXPECT_EQ(GenerateMnemonic(bytes), cases[i].mnemonic);
  ...
  EXPECT_EQ(GenerateMnemonic(std::vector<uint8_t>(8)), "");
  EXPECT_EQ(GenerateMnemonic(std::vector<uint8_t>(64)), "");
  // Random generated entropy
  EXPECT_NE(GenerateMnemonic(std::vector<uint8_t>(0)), "");
  EXPECT_NE(GenerateMnemonic(std::vector<uint8_t>(0)),
            GenerateMnemonic(std::vector<uint8_t>(0)));
}
